                ...
                for Scenario = Scenarios
                    for Data_impute = Data_imputes
                        ...
                        for a = range_vals
                            ...
                            measuredData = cell(size(detdCars,2),2);
                            measuredData(cellfun(@isempty,measuredData)) = {zeros(7,length(Time))};
                            ...
                            for j = 1:nVehIds
                                ...
                                if  ~isempty(measuredData{j,1})
                                    ...
                                    TrfID = num2str(j-1);
                                    if TrfID == '0'
                                        Title_3 = ['Ground Truths Velocity Vs Camera Measurements at ',Param{1},' ', num2str(a),'m for Target car',' ',rd];
                                    else
                                        Title_3 = ['Ground Truths Velocity Vs Camera Measurements at ',Param{1},' ', num2str(a),'m for Surround Car_', TrfID,' ',rd];
                                    end
                                    ...
                                    figure;
                                    if TrfID == '0'
                                        Title_3 = ['Ground Truths Vs Camera-based Velocity at ',Param{1},' ', num2str(a),' for Target Car'];
                                    else
                                        Title_3 = ['Ground Truths Vs Camera-based Velocity at ',Param{1},' ', num2str(a),' for Surround Car_', TrfID];
                                    end
                                    ...
                                    TrfID = num2str(j-1);
                                    if TrfID == '0'
                                        Title_4 = ['Ground Truths Acceleration Vs Camera Measurements at ',Param{1},' ', num2str(a),'m for Target car',' ',rd];
                                    else
                                        Title_4 = ['Ground Truths Acceleration Vs Camera Measurements at ',Param{1},' ', num2str(a),'m for Surround Car_', TrfID,' ',rd];
                                    end
                                    ...
                                    figure;
                                    if TrfID == '0'
                                        Title_4 = ['Ground Truths Vs Camera-based Acceleration at ',Param{1},' ', num2str(a),' for Target Car'];
                                    else
                                        Title_4 = ['Ground Truths Vs Camera-based Acceleration at ',Param{1},' ', num2str(a),' for Surround Car_', TrfID];
                                    end
                            ...
                            for t = 1:nVehIds
                                if  ~isempty(measuredData{t,1})% Check if the vehicle is detected
                                    ...
                                    trajTr = [10*Time', measurements{t,2}', measurements{t,1}',  measurements{t,5}',  measurements{t,6}', measurements{t,7}', measurements{t,8}', measuredData{t,2}(7,:)',ones(length(Time),1)*2];% [time, local x, local y, v_Vel, v_Acc, lane id, v_type] correspond to the Target car (always given traffic name 'T00' in CM)
...
for k = 1:length(trajCM(:,1))
    ...
    if ub==ind || lb ==ind || trajCM(ub,4)==0 ||trajCM(lb,4)==0 % If current index is the start OR the end of the recording OR there's no longitudinal
        ...
        trajCM(k,13) =1; % longitudinal maneuver is categorised as 'Normal speed' and adds it to a new column_8. This is questionable due to occlusions
        % when using camera
    else
